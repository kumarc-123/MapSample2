...
static NSString *kBMRingStrokeAnimationKey = @"bmmaterialdesignspinner.stroke";
static NSString *kBMRingRotationAnimationKey = @"bmmaterialdesignspinner.rotation";
...
{
    Spinner *spinner = [[self alloc] initWithView:view isMini:isMIni];
...
{
    Spinner *hud = [self HUDForView:view];
    if (hud != nil) {
...
+ (NSUInteger)hideAllHUDsForView:(UIView *)view {
    NSArray *huds = [Spinner allHUDsForView:view];
    for (Spinner *hud in huds) {
        hud.removeFromSuperViewOnHide = YES;
        [hud stopAnimating];
    }
    return [huds count];
...
+ (Spinner *)HUDForView:(UIView *)view {
    NSEnumerator *subviewsEnum = [view.subviews reverseObjectEnumerator];
    for (UIView *subview in subviewsEnum) {
        if ([subview isKindOfClass:self]) {
            return (Spinner *)subview;
        }
    }
    return nil;
}
...
- (void)startAnimating {
    ...
    [self.progressLayer addAnimation:animation forKey:kBMRingRotationAnimationKey];
    ...
    [self.progressLayer addAnimation:animations forKey:kBMRingStrokeAnimationKey];
...
{
    [self.progressLayer removeAnimationForKey:kBMRingRotationAnimationKey];
    [self.progressLayer removeAnimationForKey:kBMRingStrokeAnimationKey];
